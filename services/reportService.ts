...
export const downloadPrincipalAttendance = async (schoolId: string, schoolName: string, principalName: string, className?: string, startDate?: string, endDate?: string) => {
    try {
        ...
        const leaveCount = filtered.filter((r:any) => r.status === 'leave').length;
        const total = filtered.length;
        
        const summary = [
            { label: 'Total Records', value: total },
            { label: 'Present %', value: total > 0 ? `${Math.round((presentCount/total)*100)}%` : '0%', color: '#10b981' },
            { label: 'Absent', value: absentCount, color: '#ef4444' },
            { label: 'Leaves', value: leaveCount, color: '#f59e0b' }
        ];
