                ...
                <div className="flex-1 overflow-y-auto w-full px-4 pb-4 no-scrollbar">
                    <div className="max-w-4xl md:max-w-7xl mx-auto w-full">
                         <div className={`space-y-3 md:space-y-0 w-full transition-opacity duration-300 pb-10 ${isRefreshing ? 'opacity-40' : 'opacity-100'}`}>
                            ...
                            {role === 'driver' && ( <div className="space-y-4"> <div className={`relative w-full rounded-[2.8rem] overflow-hidden shadow-xl ${!isSchoolActive ? 'bg-rose-50 dark:bg-rose-950/20 border-2 border-rose-100 dark:border-rose-900/30' : isTripActive ? 'bg-brand-600' : 'bg-brand-500/10 dark:bg-brand-500/5 border border-brand-500/10 dark:border-white/5'}`}><div className={`transition-all duration-500 p-7 sm:p-8 ${isTripActive ? 'space-y-10' : 'space-y-0'}`}><div className="flex items-center justify-between w-full"><div className="flex items-center gap-4 sm:gap-5 flex-1 min-w-0"><div className={`w-16 h-16 sm:w-20 sm:h-20 rounded-[1.8rem] flex items-center justify-center shadow-xl relative shrink-0 ${!isSchoolActive ? 'bg-rose-500 text-white' : isTripActive ? 'bg-white text-brand-600' : 'bg-brand-500 text-white'}`}>{isTripActive ? (<div className="relative flex items-center justify-center"><div className="absolute inset-[-4px] border-4 border-brand-200 border-t-brand-600 rounded-full animate-spin"></div><Truck size={32} strokeWidth={2.5} /></div>) : (<Play size={36} fill="currentColor" strokeWidth={0} className={!isSchoolActive ? 'text-white' : ''} />)}</div><div className="space-y-0.5 truncate text-left"><h3 className={`font-black uppercase text-base sm:text-xl tracking-tight leading-tight ${!isSchoolActive ? 'text-rose-600' : isTripActive ? 'text-white' : 'text-slate-800 dark:text-white'}`}>{isTripActive ? 'LIVE TRACKING ON' : 'START SCHOOL TRIP'}</h3><p className={`text-[10px] font-black uppercase tracking-[0.2em] opacity-80 truncate ${!isSchoolActive ? 'text-rose-400' : isTripActive ? 'text-brand-50' : 'text-slate-400'}`}>{isTripActive ? (isSendingLocation ? 'TRANSMITTING...' : 'BROADCASTING LOCATION') : 'System Check Ready'}</p></div></div><button onClick={isTripActive ? handleStopTrip : handleStartTrip} disabled={!isSchoolActive} className={`w-14 h-14 sm:w-16 sm:h-16 rounded-full flex items-center justify-center shadow-2xl transition-all active:scale-90 shrink-0 z-[100] relative cursor-pointer ${!isSchoolActive ? 'bg-rose-100 text-rose-400 opacity-50' : isTripActive ? 'bg-white text-rose-600' : 'bg-brand-600 text-white border-4 border-brand-500/20'}`}>{isTripActive ? <Square size={20} fill="currentColor" className="text-rose-600" /> : <ChevronRight size={28} strokeWidth={3.5} />}</button></div>{isTripActive && (<div className="pt-6 border-t border-white/10 flex items-center justify-between premium-subview-enter"><div className="flex items-center gap-2.5"><div className={`w-2.5 h-2.5 rounded-full ${isSendingLocation ? 'bg-emerald-400 scale-125 shadow-[0_0_10px_rgba(52,211,153,1)]' : 'bg-white animate-pulse'}`}></div><span className={`text-[10px] font-black uppercase tracking-widest text-white/80`}>{isSendingLocation ? 'SATELLITE SYNC ACTIVE' : 'AUTO-SYNC EVERY 1 MINUTE'}</span></div><div className="opacity-40 text-white"><MoreHorizontal size={24} /></div></div>)}</div></div> <div onClick={() => isSchoolActive ? setIsLeaveModalOpen(true) : showLockedFeature('school')} className={`glass-card p-5 rounded-[2.5rem] flex items-center justify-between active:scale-[0.98] transition-all cursor-pointer shadow-sm border-slate-100 dark:border-white/5 ${!isSchoolActive ? 'bg-rose-50 dark:bg-rose-950/20 border-rose-100 dark:border-rose-900/30' : ''}`}><div className="flex items-center gap-4"><div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-inner ${!isSchoolActive ? 'bg-rose-500 text-white' : 'bg-brand-500/10 text-brand-600'}`}><CalendarRange size={28} /></div><div className="text-left"><h3 className={`font-black uppercase text-base leading-tight ${!isSchoolActive ? 'text-rose-600' : 'text-slate-800 dark:text-white'}`}>{t('apply_leave')}</h3><p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Staff Request Portal</p></div></div>{!isSchoolActive ? <Lock size={20} className="text-rose-400" /> : <ChevronRight size={22} className="text-slate-200" />}</div> </div> )}
                            {(role === 'parent' || role === 'student') && ( data?.student_id ? ( <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4"> <div onClick={() => isSchoolActive ? setParentStack(prev => [...prev, 'attendance_history']) : showLockedFeature('school')} className={`glass-card p-5 rounded-[2.5rem] flex items-center justify-between active:scale-[0.98] transition-all cursor-pointer shadow-sm ${!isSchoolActive ? 'bg-rose-50 dark:bg-rose-950/10 border-rose-100 dark:border-rose-900/20' : ''}`}> <div className="flex items-center gap-4"> <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-inner ${!isSchoolActive ? 'bg-rose-500 text-white' : 'bg-brand-500/10 text-brand-600'}`}><UserCheck size={28} /></div> <div><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-0.5">{t('attendance_status')}</p><h4 className={`text-base font-black uppercase leading-tight ${!isSchoolActive ? 'text-rose-600' : 'text-slate-800 dark:text-white'}`}>{t('current')}: <span className={data?.today_attendance === 'present' ? 'text-emerald-500' : data?.today_attendance === 'absent' ? 'text-rose-500' : 'text-brand-500'}>{data?.today_attendance ? t(data.today_attendance) : t('waiting')}</span></h4></div> </div> {!isSchoolActive ? <Lock size={18} className="text-rose-400" /> : <ChevronRight size={20} className="text-slate-300" />} </div> {[ { key: 'apply_leave', icon: <CalendarRange size={28} />, title: t('apply_leave'), sub: 'Absence Request Portal' }, { key: 'live_transport', icon: <Truck size={28} />, title: t('live_transport'), sub: 'Real-time Route Monitor' }, { key: 'daily_tasks', icon: <BookOpen size={28} />, title: t('daily_tasks'), sub: 'Check Current Assignments' } ].map((it, idx) => { const isLocked = !isSchoolActive || !isUserActive; return ( <div key={idx} onClick={() => isLocked ? (isSchoolActive ? showLockedFeature('parent') : showLockedFeature('school')) : setParentStack(prev => [...prev, it.key])} className={`glass-card p-5 rounded-[2.5rem] flex items-center justify-between active:scale-[0.98] transition-all cursor-pointer shadow-sm ${isLocked ? 'bg-rose-50 dark:bg-rose-950/10 border-rose-100 dark:border-rose-900/20' : ''}`}> <div className="flex items-center gap-4"><div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-inner ${isLocked ? 'bg-rose-500 text-white' : 'bg-brand-500/10 text-brand-600'}`}>{it.icon}</div><div className="text-left"><h3 className={`font-black uppercase text-base leading-tight ${isLocked ? 'text-rose-600' : 'text-slate-800 dark:text-white'}`}>{it.title}</h3><p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{it.sub}</p></div></div> {isLocked ? <Lock size={18} className="text-rose-400" /> : <ChevronRight size={20} className="text-slate-300" />} </div> ); })} </div> ) : ( <div className="p-8 text-center bg-slate-50 dark:bg-white/5 rounded-[2.5rem] border border-slate-100 dark:border-white/5"><div className="w-16 h-16 bg-slate-200 dark:bg-white/10 rounded-full flex items-center justify-center mx-auto mb-4 text-slate-400"><AlertCircle size={32} /></div><h3 className="font-black uppercase text-slate-600 dark:text-slate-400 mb-2">Profile Not Linked</h3><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Ask Admin to link your profile.</p></div> ) )}
                         </div>
                    </div>
                </div>
            </>
